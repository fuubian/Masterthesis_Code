...
class MetricLoader:
    ...
    def load_metric(metric_name):
        if metric_name.lower() == config.LLM_ACCURACY_METRIC:
            return llm_accuracy.LLMAccuracy()
        if metric_name.lower() == config.VQA_MQM_METRIC:
            return vqa_mqm.VQA_MQM()
        if metric_name.lower() == config.METEOR_METRIC:
            return meteor.MeteorMetric()
        if metric_name.lower() == config.BERT_SCORE_METRIC:
            return bert_score.BertScoreMetric()
        if metric_name.lower() == config.CIDER_METRIC:
            return cider.CiderMetric()
        if metric_name.lower() == config.BLEU_METRIC:
            return bleu.BleuMetric()
        if metric_name.lower() == config.ROUGE_METRIC:
